...
class BusinessPartnerRegister extends DrawerContent {
  final String title;
  BusinessPartnerRegister({Key key, this.title});
  @override
  _BusinessPartnerRegisterState createState() => _BusinessPartnerRegisterState();
}

class _BusinessPartnerRegisterState extends State<BusinessPartnerRegister> {
                          ...
                          onPressed: () async {
                            if(_image == null){
                              Fluttertoast.showToast(
                                  msg: AppLocalizations.of(context).translate("selectProfile")?? Const.selectProfile,
                              ...
                              Fluttertoast.showToast(
                                  msg:  AppLocalizations.of(context).translate("mobileNOError")?? Const.mobileNumber,
                                  gravity: ToastGravity.BOTTOM,
                                  toastLength: Toast.LENGTH_LONG);
                            } else if (ageController.text == '') {
                              Fluttertoast.showToast(
                                  msg:  AppLocalizations.of(context).translate("ageError")?? Const.ageError,
                                  gravity: ToastGravity.BOTTOM,
                                  toastLength: Toast.LENGTH_LONG);
                            } else {

                              bool data = await PartnerRegisterService.validateNumber("+91${mobileController.text}");
                              if (data) {
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: Column(
            children: <Widget>[
              TextField(
                decoration: InputDecoration(labelText: AppLocalizations.of(context).translate("name")?? Const.name),
              ...
              TextField(
                ...
                decoration: InputDecoration(labelText: AppLocalizations.of(context).translate("mobileNumber")?? Const.mobileNumber),
              ),
              TextField(
                decoration:
                InputDecoration(labelText:AppLocalizations.of(context).translate("alterMobile")?? Const.alterMobile ),
                maxLength: 10,
              ...
              TextField(
                ...
                decoration: InputDecoration(labelText: AppLocalizations.of(context).translate("age")?? Const.age),
                keyboardType: TextInputType.number,
                inputFormatters: <TextInputFormatter>[
                  WhitelistingTextInputFormatter.digitsOnly,
                ],
              ...
              Consumer<PartnerRegisterProvider>(builder: (_, notifier, __) {
                    ...
                    Column(
                      children: <Widget>[
                        Padding(
                          ...
                          child: Text(
                            AppLocalizations.of(context).translate("onetimePass")?? Const.onetimePass,
  ...
  void _onFormSubmitted(BuildContext context) async {
    ...
    _firebaseAuth.signInWithCredential(_authCredential)
      ...
      if (value.user != null) {
        ...
        var provider = Provider.of<PartnerRegisterProvider>(
            context, listen: false);

        var image = await provider.uploadImage(
            _image, "Business Partner/${value.user.uid}/");

        var agent_id = await PrefManager.getUserId();

        await provider.addPartnerProfile(
            value.user.uid,
            nameController.text,
            ...
            "+91${mobileController.text}",agent_id,_character.toString().substring(17));
        ...
        Fluttertoast.showToast(
            msg:  AppLocalizations.of(context).translate("businessPartnerCreted")?? Const.businessPartnerCreted,
            toastLength: Toast.LENGTH_SHORT,
            gravity: ToastGravity.BOTTOM,
            timeInSecForIos: 1,
            textColor: Colors.black38,
  ...
  Route _createRoute(String uid){
    ...
    return PageRouteBuilder(
      pageBuilder: (context, animation, secondaryAnimation) => HomePage(fireuser: uid),
